---
title: "ID指定でオブジェクトを取得する方法【React】"
createAt: "2021-06-02 19:00"
updateAt: null
tags: ["React"]
---

## ■はじめに

以前、「ボタン押下時にテキストの値を取得する方法【React】」でテキストの変更時処理で変数に値を保持して操作していました。

ここでは、オブジェクトを取得することで、必要な時に最新の値が取得できるようになります。

どちらが良いかは、好みな気もしますが、個人的には、こちらの方法の方がしっくりきます。

## ■手順

まずは、App.jsの[ソースコード](http://d.hatena.ne.jp/keyword/%A5%BD%A1%BC%A5%B9%A5%B3%A1%BC%A5%C9)の全量は以下

```js
import React from 'react';

class App extends React.Component{
  render()
  {
    function btnClick() {
      alert(document.getElementById('target').value);
    }
    return (

# Hello World

        <input type="text" id="target"/>

        <button onClick={btnClick}>ボタン</button>

    );
  }
}

export default App;
```

以下のようにすることで、IDを指定してオブジェクトを取得できます。

```js
document.getElementById('[id名]')
```

取得したオブジェクトの「.[value](http://d.hatena.ne.jp/keyword/value)」でテキストフィールドの場合は入力されている値を取得できます。
