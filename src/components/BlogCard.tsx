...
export default function BlogCard({ post }: Props) {
  ...
  return (
    <article className="group relative rounded-xl border border-slate-700 bg-slate-800 p-6 transition-all hover:border-slate-500" data-testid="blog-card">
      <Link
        href={`/blog/${slug}`}
        className="block before:absolute before:inset-0 before:rounded-xl"
      >
        <h2 className="mb-2 text-xl font-semibold text-slate-100 [overflow-wrap:anywhere] group-hover:text-blue-400">
          {frontmatter.title}
        </h2>
      </Link>
